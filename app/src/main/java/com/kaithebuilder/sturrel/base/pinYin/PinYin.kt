...
class PinYin private constructor() {
    companion object {
        val instance: PinYin by lazy {
            PinYin()
        }
    }

    private val unicodeToPinyinMap: Map<String, String> =
        unicode_to_hanyu_pinyin.trimIndent().split("\n").associate {
        val (unicode, pinyin) = it.split(" ")
        unicode to pinyin
    }

    private fun getPinyin(unicode: String): Array<String> {
        val pinyinString = unicodeToPinyinMap[unicode] ?: return emptyArray()
        val pinyinList = pinyinString.substring(1, pinyinString.length - 1).split(",")
        return pinyinList.toTypedArray()
    }

    private fun formattedPinYin(pinyin: String): String {
        val tone = pinyin.last().toString().toInt()
        val vowels = listOf("a", "e", "i", "o", "u", "u:")
        ...
        val formattedChar = characterMap[pinyinChar.toString()]?.get(tone - 1) ?: pinyinChar.toString()
    ...
    fun getPinyinString(input: String, format: Boolean = true): String {
        ...
        for (char in input) {
            val unicodeScalar = char.code.toString(16).uppercase()
...
val characterMap = mapOf(
    ...
    "u:" to arrayOf("ǖ", "ǘ", "ǚ", "ǜ", "ü")
)

val unicode_to_hanyu_pinyin = """
3007 (ling2)
4E00 (yi1)
4E01 (ding1,zheng1)
4E02 (kao3)
